...
export class AdminService {
  ...
  static async getSOSMetrics(): Promise<Record<string, unknown>> {
    ...
    const [
      ...
      totalVetSearches,
      systemLocationSearches,
      manualLocationSearches,
      amapSuccessSearches,
      noResultSearches,
      amapFailedFallbackSearches,
      dbFallbackSearches,
      localVetClinics,
    ] = await Promise.all([
      ...
      prisma.sosSearchLog.count(),
      prisma.sosSearchLog.count({ where: { source: 'SYSTEM_LOCATION' } }),
      prisma.sosSearchLog.count({ where: { source: 'MANUAL_LOCATION' } }),
      prisma.sosSearchLog.count({ where: { resultStatus: 'AMAP_SUCCESS' } }),
      prisma.sosSearchLog.count({ where: { resultStatus: 'NO_RESULTS' } }),
      prisma.sosSearchLog.count({ where: { resultStatus: 'AMAP_FAILED_DB_FALLBACK' } }),
      prisma.sosSearchLog.count({ where: { resultStatus: 'DB_FALLBACK' } }),
      prisma.vetClinic.count(),
    ]);
